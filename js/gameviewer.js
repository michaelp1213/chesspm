...
function waitForImagesReady() {
  if (allImagesLoaded()) {
    try {
      chrome.webview.postMessage("BOARD_READY");
    } catch (e) { }
  } else {
    setTimeout(waitForImagesReady, 50); // 🔁 retry
  }
}
...
highLight = function (sq) {
  board.highlightSquare(sq);
}
highLightView = function (sq) {
  highLight(sq);
  try {
    window.chrome.webview.postMessage("highl " + sq);
  } catch (ee) { }
}
async function doUciMove(mv) {
  var tf = board.doUciMove(mv);
  // already changed !!
  //if (tf)
  //  board.changePlayColor();
  try {
  if (tf && p_wview==1)
    window.chrome.webview.postMessage("mdone:" + mv);
  else if (p_wview==1)
      window.chrome.webview.postMessage("merr:" + mv);
  } catch (ee) { }
}
async function doUciMoveSpecial(mv) {
  var tf = board.doUciMove(mv);
  if (mv == "e1g1")
    board.doUciMove('h1f1');
  else if (mv == "e8g8")
    board.doUciMove('h8f8');

  else if (mv == "e1c1")
    board.doUciMove('a1d1');

  else if (mv == "e8c8")
    board.doUciMove('a8d8');
...
async function doUciMoveAndAnswer(mv, mvans) {
  var tf = board.doUciMove(mv);

  if (tf && p_wview == 1) { 
    await sleep(500); 
    var tf1 = board.doUciMove(mvans);
    window.chrome.webview.postMessage("mdoneser:" + mv+","+mvans);
  }
  else if (p_wview == 1)
    window.chrome.webview.postMessage("merr:" + mv);
}
// as next but without prev move, may be also without sq
async function replaceViewLastFen(fen2, sq, setside, rback) {
  try {
    p_wview = 1; // ?
    var col = getSideToMove(fen2);

    if (!setside || setside == 0) { }
    else if (setside == 2 || (setside == 1 && (p_flip == '1' || p_flip == '2'))) {
      if (col == 'b')
        board.blackdown();
      else
        board.whitedown();
    ...
    board.setPlayColor(col);
    await sleep(10);     // 50 ms
    if(sq && sq!='')
      highLight(sq);  
    if (!rback)
      rback = '';
    if (p_wview == 1)
      window.chrome.webview.postMessage("loadedfen" + rback+" c:" + col);
  } catch (e) {
    console.error(e);
  }
}
async function replaceViewLastMove(fen1, fen2, sq,setside,f) {
  try {
    p_wview = 1; // ?
    var col = getSideToMove(fen2);     
    ...
    // FINAL STEP
    if ( p_wview == 1)
      window.chrome.webview.postMessage("loadedfen c:" + col);
  } catch (e) {
    console.error(e);
  }
}
async function setPlayColor(col) {
	 board.setPlayColor(col);
}
function sleep(ms) {
...
function doReplayFens(fenArr, delay = 1000) {
  if (replayTimer)
    clearTimeout(replayTimer);

  timerBoardDisplay(0, fenArr, delay);
}

function timerBoardDisplay(counter, fenArr, delay) {
  if (counter >= fenArr.length) {
    replayTimer = null;
	  // ✅ replay finished HERE
	  if( p_wview == 1)
        try {
            window.chrome.webview.postMessage("finreplay");
        } catch (ee) {}

    return;
  }

  replaceFen(fenArr[counter]);

  replayTimer = setTimeout(() => {
    timerBoardDisplay(counter + 1, fenArr, delay);
  }, delay);
}

doReplayFens2  = function (fenArr) {
	timerBoardDisplay( 0,  fenArr);
}	
function timerBoardDisplay( counter, fenArr)
{
  if (counter >= fenArr.length)
    return;
 
   replaceFen(fenArr[counter]);
  // board.setFen(fenArr[counter], 'w');

  t = setTimeout(function() {
    timerBoardDisplay( counter + 1, fenArr);
  }, 1000);
}
...
replaceOnlyFen = function (fen) {
  p_wview = 1;
  replaceFen(fen);
  var col = getSideToMove(fen);
  //alert(col);
  board.setPlayColor(col);
  board.setDraggable(true);
  try {
    window.chrome.webview.postMessage("loadedfen");
  } catch (ee) { }
  //}
}
replaceViewFen = function (fen) {
  p_wview = 1;
  replaceFen(fen);
  var col = getSideToMove(fen);
  //alert(col);
  board.setPlayColor(col);
  if (p_flip == '1' || p_flip == '2') {
    if (col == 'b')
      board.blackdown();
    else
      board.whitedown();
