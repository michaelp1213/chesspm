...
function DoSave(pgn, pgn_type, hidemov, saveim, pswreq, psw, bcol, totemp, fen, hdr, sethdr, pri, keys) {
  ...
  // 👉 build request object (NO escaping!)
  var request = {
    pgn: pgn,
    pgnType: pgn_type,
    hideMov: hidemov,
    pswReq: pswreq,
    psw: psw,
    bcol: bcol ? parseInt(bcol) : null,
    fen: fen,
    hdr: hdr,
    keys: keys,
    commands: '',
    mafrid: ($("#saveparam").val() || 'Event'),
    saveMode: SaveUpd,     // 👈 1 / 2 / 3
    src: (totemp ? 'temp' : (p_dbsrc || ''))
  };
  ...
  $.ajax({
    type: "POST",
    url: "https://www.chesspm.com/api/PuzzleObject/save",
    ...
    success: function (data) {
      ...
      if (SaveUpd == 3) {
        SaveOrReplaceMessage(11, data, 'Saved', 0, 0, 0, 0);
      }
      else {
        if (saveim && saveim == 1) {
          if (data > 0)
            saveTheBoardImageToServerDo(data, 1);
          else if (data == -2)
            alert('Bad password!');
        }
        else if (boardscreen && boardscreen == 1) {
          afterSaveBoard(12, data, 'Saved', 0, g_sq, 0, 0, 1, totemp);
        }
        else {
          SaveOrReplaceMessage(12, data, 'Saved', 0, g_sq, 0, 0);
        }
      }
    },
...
function DoReplace(pgn, id, pgn_type, hidemov, pswreq, psw, cmdnew, bcol, fen, hdr, totemp, pri, keys) {
  ...
  var request = {
    ...
    keys: keys,
    saveMode: 2,                    // 👈 REPLACE
    src: (totemp == 1 || p_dbsrc == 'temp') ? 'temp'
      : (p_dbsrc == 'my') ? 'my'
        : (totemp == 2) ? 'diag'
          : ''
  };
  ...
  $.ajax({
    type: "POST",
    url: "https://www.chesspm.com/api/PuzzleObject/save1",
    data: JSON.stringify(request),
    contentType: 'application/json',
    ...
    success: function (data) {
      ...
      if (data <= 0) {
        $('#save_res1').html('Error ' + data + ' in replacing id: ' + id);
        return;
      }
      ...
      if (boardscreen && boardscreen == 1) {
        afterSaveBoard(12, data, 'Replaced', 0, g_sq, 0, 0, 2, totemp);
      }
      else {
        SaveOrReplaceMessage(12, data, 'Replaced', 0, g_sq, 0, 0);
      }
    },
