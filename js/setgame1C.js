...
function game_list_html_load(gn, annum, p_hm) {
  ...
  // 👉 build URL
  var url = "https://www.chesspm.com/api/PuzzleObject/pgnlist" +
    "?fn=" + encodeURIComponent(p_fname) +
    "&id=1";
...
function game_html_load(gn, annum, p_hm) {
  ...
  // 👉 build URL
  var url = "https://www.chesspm.com/api/PuzzleObject/pgngame" +
    "?fn=" + encodeURIComponent(p_fname) +
    "&gamenum=" + gn;
...
function game_text_load(gn, p_hm) {
  ...
  // 👉 build URL
  var url = "https://www.chesspm.com/api/PuzzleObject/game" +
    "?id=" + gn +
    "&src=" + (p_src || "") +
    "&score=" + sc +
    "&lat=" + p_lat +
    "&psw=" + encodeURIComponent(p_psw || "") +
  ...
  $.ajax({
    ...
    success: function (game) {
      ...
      if (boardscreen && boardscreen == 1) {
        ...
        (p_solve && p_solve > 0 && p_solve < 10) ||
        ((p_solve == 10 || p_solve == null) && game.HideMov > 10 && game.HideMov < 20)
      ) {
        if (p_solve == null)
          p_solve = 10;
...
//api
async function game_pgn_load(pgn, pgn_type, tostart, act) {
  ...
  try {
    const response = await fetch("https://www.chesspm.com/api/PuzzleObject/diagbyid/", {
      method: "POST",
      headers: {
...
function game_html_db_load(gn, p_hm) {
  ...
  // 👉 build URL
  var url = "https://www.chesspm.com/api/PuzzleObject/" + method + "/" + gn;

  if (p_psw && p_psw !== '') {
    url += "?psw=" + encodeURIComponent(p_psw);
  }
  ...
  $.ajax({
    ...
    success: function (game) {
      ...
      // ❗ NO JSON.parse here anymore
      // game is already object
...
function game_html_db_list_load(gn, gnto, p_hm, p_glist, p_load, p_search) {
  ...
  // 👉 glist logic (like before)
  var glist = gn;

  if ((p_glist && p_glist != '') || (p_search && p_search != '')) {
    glist = p_glist ? p_glist : "";
  }

  // 👉 build URL
  var url = "https://www.chesspm.com/api/PuzzleObject/" + method + "/" + encodeURIComponent(glist);

  // 👉 optional query params (if you later support them in API)
  // url += "?idto=" + gnto + "&lat=" + p_lat + "&load=" + p_load + "&psw=" + p_psw;
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (gameList && gameList.length > 0) {
        ...
        var selectedGame = gameList[parseInt(p_chooser) - 1];

        if (p_solve && p_solve > 0 && p_solve < 10) {
          setPuzzle(selectedGame, 1);
        }
        else {
          setNewGame(selectedGame, p_hm);
        }
      }
      else {
        $('.timer').hide();
...
function game_html_db_list_load_by_id(p_hm, p_glist, p_load, p_tourn, xgn, dbsrc) {
  ...
  // 👉 build URL
  var url = "https://www.chesspm.com/api/PuzzleObject/gamelistbyid?" +
  ...
  if (psw)
    url += "&psw=" + encodeURIComponent(psw);

  if (p_glist)
    url += "&glist=" + encodeURIComponent(p_glist);

  if (dbsrc)
    url += "&db=" + encodeURIComponent(dbsrc);
